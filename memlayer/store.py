...
class MemoryStore:
    ...
    def search(self, query: str, top_k: int = 5) -> list[dict]:
        ...
        # rank inside the FTS index, filtering by user/status BEFORE the
        # LIMIT so one profile's dense matches can't starve another's
        rows = self._db.execute(
            f"""WITH ranked AS (
                ...
                ORDER BY r.score""",
            (q, self.user_id, max(top_k * 8, 24))).fetchall()
        results = []
        for r in rows:
        ...
        if embedder is not None and results:
            ...
            # two-pass stable sort: within near-tied BM25 scores, prefer
            # NEWER memories and HIGHER confidence
            results.sort(key=lambda m: (m["created_at"], m["confidence"]),
                         reverse=True)
